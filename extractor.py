...
"""
import sys, re, json, time, zlib, collections
import onec_dtools
...
class TwinExtractor:
    ...
    def objects(self):
        """Все объекты, дающие формы+акторы: справочники → документы → строки документов → регистры сведений."""
        out = []
    ...
    def fn(ex, cursor, push, budget):
        ...
        while i < len(objs):
            ...
            push({"op": "create_form", "ref": o["ref_form"], "title": o["title"],
                  "color": o["color"], "fields": o["form_fields"]})
        return {"obj": i}, True
    return fn
    ...
    while i < len(regs):
        ...
        while r < len(rows):
            ...
            for col, name, kind_cur in reg["resources"]:
                ...
                push({"op": "transfer", "ref": "tx%d-%d-%s" % (reg["idx"], ri, col),
...
STAGES = [
    ...
    ("forms_lines", _forms("vt")),       # табличные части документов (строки)
    ...
    ("account_txns", stage_account_txns),       # движения регистров → balanced transfers
    ("links", stage_links),
]
...
def window(file_url, stage, cursor=None, max_bytes=1_400_000, max_secs=25, _ex=None):
